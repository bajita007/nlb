...
export function AdminSidebar({ currentAdmin }: AdminSidebarProps) {
  ...
  const handleMenuClick = () => {
    console.log("Menu clicked, current state:", open)
    setOpen(!open)
  }
  ...
  return (
    <>
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white border-b px-4 py-3 flex items-center justify-between shadow-sm">
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
            <Button variant="outline" size="icon" onClick={handleMenuClick} className="hover:bg-gray-100">
              <Menu className="h-4 w-4" />
              <span className="sr-only">Toggle menu</span>
            </Button>
          </SheetTrigger>
          <SheetContent side="left" className="p-0 w-80 max-w-80">
            <SidebarContent />
          </SheetContent>
        </Sheet>
      ...
      {/* Desktop Sidebar */}
      <div className="hidden lg:flex lg:w-80 lg:flex-col lg:fixed lg:inset-y-0 lg:z-50 lg:bg-white lg:border-r">
        <SidebarContent />
      </div>
    </>
  )
